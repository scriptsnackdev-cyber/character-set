import { SBTI_TYPES } from './sbti-data';
...
export async function downloadSBTIWithFrame(imgSrc: string, sbtiId: string, characterName?: string): Promise<void> {
  const img = new Image();
  ...
  const sbtiData = SBTI_TYPES.find(s => s.id === sbtiId);
  const accentColor = sbtiData?.color || '#ff69b4';
  ...
  // Draw card name
  const displayText = `${sbtiId} - ${sbtiData?.nameTh || ''}`.toUpperCase();
  ctx.save();
  ...
  const safeId = sbtiId.replace(/[^a-zA-Z0-9]/g, '_');
  link.download = `CHIBI_SBTI_${safeId}_${characterName || 'CAT'}.png`;
